	...
	prototype: {
		...
		check: function( element ) {
			...
			for (var method in rules) {
				var rule = { method: method, parametros: rules[method] };
				try {
					var result = $.validator.methods[method].call( this, element.value.replace(/\r/g, ""), element, rule.parametros );
		...
		formatAndAdd: function( element, rule ) {
			...
			if ( typeof message == "function" ) {
				...
				message = jQuery.format(message.replace(theregex, '{$1}'), rule.parametros);
			}			
			this.errorList.push({
				message: message,
				element: element
